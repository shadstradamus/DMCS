"""Core taxonomy data structures and query functionality"""
...
class Industry:
    ...
    def __repr__(self):
        return f"{self.id} — {self.label} ({len(self.sectors)} sectors, {self.taxonomy})"
...
class Taxonomy:
    """DMCS Taxonomy - main interface for querying classification data"""

    def __init__(self, data_path: Optional[Path] = None):
        """Initialize DMCS taxonomy from the packaged JSON or a custom path."""

        if data_path is None:
            data = self._load_packaged_taxonomy()
        else:
    ...
    @staticmethod
    def _load_packaged_taxonomy() -> Dict[str, Any]:
        """Load taxonomy JSON bundled with the package."""

        try:
            json_text = resources.files('dmcs_sdk.data').joinpath('taxonomy.json').read_text(encoding='utf-8')
        except AttributeError:  # Python 3.8 compatibility
            with resources.open_text('dmcs_sdk.data', 'taxonomy.json', encoding='utf-8') as f:
                return json.load(f)
    ...
    def filter_by_taxonomy(self, taxonomy: str) -> List[Industry]:
        """
        Get all industries in a specific taxonomy
        ...
        """
        return [i for i in self.industries if i.taxonomy == taxonomy]

    def get_p_tax(self) -> List[Industry]:
        """Get all P-TAX (Primary Taxonomy) industries (01-12)"""
        return self.filter_by_taxonomy("P-TAX")

    def get_d_tax(self) -> List[Industry]:
        """Get all D-TAX (Digital Asset Taxonomy) industries (13)"""
        return self.filter_by_taxonomy("D-TAX")
